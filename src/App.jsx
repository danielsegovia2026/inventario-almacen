...
import {
  ...
  ShoppingCart, TrendingUp, Minus, Receipt, Hash, Zap, Layers, Check
} from "lucide-react";
...
export default function App() {
  ...
  const reportStats = useMemo(() => {
    ...
    let allTotal = 0, allCost = 0, allProfit = 0;
    const productTotals = {};

    for (const s of sales) {
      const d = new Date(s.created_at);
      const total = Number(s.total);
      const cost = Number(s.cost_total);
    ...
    return { todayTotal, todayProfit, monthTotal, monthProfit, allTotal, allCost, allProfit, topProducts };
  }, [sales]);
  ...
  async function quickMove(productId, type) {
    await registerMovement(productId, type, 1, "Carga rapida");
  }

  async function applyBulkChanges(changes) {
    const qtyMap = {};
    for (const p of products) qtyMap[p.id] = p.qty;

    for (const change of changes) {
      const amount = Number(change.qty);
      if (!amount || amount <= 0) continue;
      const current = qtyMap[change.productId] ?? 0;
      const newQty = change.type === "in" ? current + amount : Math.max(0, current - amount);
      qtyMap[change.productId] = newQty;
      await supabase.from("products").update({ qty: newQty }).eq("id", change.productId);
      await supabase.from("movements").insert({ product_id: change.productId, type: change.type, qty: amount, note: "Carga en lote" });
    }
    await fetchAll();
  }
  ...
  async function deleteSale(sale) {
    const confirmed = window.confirm("¿Eliminar esta venta? Se va a devolver el stock descontado.");
    if (!confirmed) return;
    ...
    for (const item of sale.sale_items || []) {
      ...
      if (product) {
        ...
        await supabase.from("products").update({ qty: newQty }).eq("id", item.product_id);
      }
    }
    ...
    <div style={{ background: PALETTE.bg, color: PALETTE.text, minHeight: "100vh", fontFamily: "Inter, system-ui, sans-serif" }} className="w-full p-4">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        onClick={() => quickMove(p.id, "out")}
                        title="Restar 1 (rapido)"
                        className="w-7 h-7 flex items-center justify-center rounded-full"
                        style={{ background: PALETTE.panelAlt }}
                      >
                        <Minus size={14} color={PALETTE.danger} />
                      </button>
                      <button
                        onClick={() => quickMove(p.id, "in")}
                        title="Sumar 1 (rapido)"
                        className="w-7 h-7 flex items-center justify-center rounded-full"
                        style={{ background: PALETTE.panelAlt }}
                      >
                        <Plus size={14} color={PALETTE.success} />
                      </button>
                      <button onClick={() => setMovementTarget({ product: p, type: "out" })} title="Cantidad personalizada">
                        <Hash size={16} color={PALETTE.textMuted} />
                      </button>
      ...
      {tab === "reportes" && (
        <div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-5">
            {[
              { label: "Total vendido", value: money(reportStats.allTotal), color: PALETTE.text },
              { label: "Total costo", value: money(reportStats.allCost), color: PALETTE.textMuted },
              { label: "Ganancia total", value: money(reportStats.allProfit), color: PALETTE.success },
            ].map((s) => (
              <div key={s.label} className="rounded-lg p-3" style={{ background: PALETTE.panelAlt, border: `1px solid ${PALETTE.border}` }}>
                <p className="text-xs mb-1" style={{ color: PALETTE.textMuted }}>{s.label}</p>
          ...
          <h3 className="display-font text-sm font-semibold mb-2">Registro de ventas</h3>
          {sales.length === 0 ? (
            <p className="text-sm py-4" style={{ color: PALETTE.textMuted }}>Aun no hay ventas registradas.</p>
          ) : (
            <div className="space-y-2">
                ...
                return (
                  <div key={s.id} className="rounded-lg p-3" style={{ background: PALETTE.panel, border: `1px solid ${PALETTE.border}` }}>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button onClick={() => deleteSale(s)} title="Eliminar venta">
                    ...
                    <div className="flex gap-4 pt-2" style={{ borderTop: `1px solid ${PALETTE.border}` }}>
                      <div>
                        <p className="text-xs" style={{ color: PALETTE.textMuted }}>Venta</p>
                        <p className="text-sm font-semibold">{money(s.total)}</p>
                      </div>
                      <div>
                        <p className="text-xs" style={{ color: PALETTE.textMuted }}>Costo</p>
                        <p className="text-sm font-semibold" style={{ color: PALETTE.textMuted }}>{money(s.cost_total)}</p>
                      </div>
                      <div>
                        <p className="text-xs" style={{ color: PALETTE.textMuted }}>Ganancia</p>
                        <p className="text-sm font-semibold" style={{ color: profit >= 0 ? PALETTE.success : PALETTE.danger }}>{money(profit)}</p>
                      </div>
                    </div>
                  </div>
                );
      ...
      {movementTarget && (
        <MovementModal
          ...
          onConfirm={(type, qty, note) => registerMovement(movementTarget.product.id, type, qty, note)}
        />
      )}
      ...
      {showFastEntry && (
        <FastEntryModal
          ...
          categories={categories}
          palette={PALETTE}
          onCancel={() => setShowFastEntry(false)}
          onQuickMove={quickMove}
          onNotFound={(code) => { setShowFastEntry(false); handleScanResult(code); }}
        />
      )}
      ...
      {showBulkEntry && (
        <BulkEntryModal
          ...
          onApply={async (changes) => { await applyBulkChanges(changes); setShowBulkEntry(false); }}
        />
      )}
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", zIndex: 50 }}>
      <div className="w-full max-w-xs rounded-lg p-4" style={{ background: palette.panel, border: `1px solid ${palette.borderStrong}` }}>
        <h3 className="display-font text-sm font-semibold mb-3">Cantidad personalizada</h3>
        <div className="flex gap-2 mb-3">
          <button
            onClick={() => setType("in")}
            className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-sm font-medium"
            style={{ background: isIn ? palette.success : "transparent", color: isIn ? "#fff" : palette.textMuted, border: `1px solid ${isIn ? palette.success : palette.border}` }}
          >
          ...
          <button
            onClick={() => setType("out")}
            className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded text-sm font-medium"
            style={{ background: !isIn ? palette.danger : "transparent", color: !isIn ? "#fff" : palette.textMuted, border: `1px solid ${!isIn ? palette.danger : palette.border}` }}
          >
...
function FastEntryModal({ products, categories, palette, onCancel, onQuickMove, onNotFound }) {
  const videoRef = useRef(null);
  const [error, setError] = useState("");
  const [manualCode, setManualCode] = useState("");
  ...
  async function act(type) {
    if (!current) return;
    await onQuickMove(current.id, type);
    setLog((prev) => [{ name: current.name, type }, ...prev].slice(0, 6));
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.75)", zIndex: 50 }}>
      <div className="w-full max-w-sm rounded-lg p-4 max-h-[92vh] overflow-y-auto" style={{ background: palette.panel, border: `1px solid ${palette.borderStrong}` }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="display-font text-sm font-semibold flex items-center gap-1"><Zap size={14} color={palette.accent} /> Carga rapida</h3>
          <button onClick={onCancel}><X size={16} color={palette.textMuted} /></button>
        </div>
        ...
        {current && (
          <div className="rounded-lg p-3 mb-3" style={{ background: palette.panelAlt, border: `1px solid ${palette.border}` }}>
            <p className="text-sm font-medium mb-1">{current.name}</p>
            <p className="text-xs mb-3" style={{ color: palette.textMuted }}>Stock actual: {current.qty}</p>
            <div className="flex gap-2">
              <button onClick={() => act("out")} className="flex-1 py-2 rounded text-sm font-medium" style={{ background: palette.danger, color: "#fff" }}>
                -1 Salida
              </button>
              <button onClick={() => act("in")} className="flex-1 py-2 rounded text-sm font-medium" style={{ background: palette.success, color: "#fff" }}>
                +1 Entrada
              </button>
            </div>
          </div>
        ...
        {notFoundCode && (
          <div className="rounded-lg p-3 mb-3" style={{ background: palette.panelAlt, border: `1px solid ${palette.danger}` }}>
            <p className="text-sm mb-2">Codigo no encontrado: <span className="mono-font">{notFoundCode}</span></p>
            <div className="flex gap-2">
              <button onClick={resume} className="flex-1 py-1.5 rounded text-sm" style={{ border: `1px solid ${palette.border}`, color: palette.textMuted }}>
                Seguir escaneando
              </button>
              <button onClick={() => onNotFound(notFoundCode)} className="flex-1 py-1.5 rounded text-sm font-medium" style={{ background: palette.accent, color: palette.accentText }}>
                Crear producto
              </button>
            </div>
          </div>
        ...
        {!current && !notFoundCode && (
          <p className="text-xs text-center py-2 mb-2" style={{ color: palette.textMuted }}>Apunta la camara al codigo de barras...</p>
        )}
        ...
        <div className="flex gap-2 mb-3">
          <input
            value={manualCode}
            onChange={(e) => setManualCode(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && manualCode.trim() && (handleCode(manualCode.trim()), setManualCode(""))}
        ...
        {log.length > 0 && (
          <div>
            <p className="text-xs mb-1" style={{ color: palette.textMuted }}>Ultimos registrados en esta sesion:</p>
            <div className="space-y-1">
              {log.map((l, idx) => (
                <div key={idx} className="flex items-center gap-2 text-xs">
                  <Check size={12} color={palette.success} />
                  <span>{l.type === "in" ? "+1" : "-1"} {l.name}</span>
                </div>
              ))}
            </div>
          </div>
...
function BulkEntryModal({ products, palette, onCancel, onApply }) {
  const [search, setSearch] = useState("");
  ...
  function setRow(productId, field, value) {
    setRows((prev) => ({ ...prev, [productId]: { ...prev[productId], [field]: value } }));
  }

  const changeCount = Object.entries(rows).filter(([, v]) => Number(v?.in) > 0 || Number(v?.out) > 0).length;

  async function handleApply() {
    const changes = [];
    for (const [productId, v] of Object.entries(rows)) {
      if (Number(v?.in) > 0) changes.push({ productId, type: "in", qty: Number(v.in) });
      if (Number(v?.out) > 0) changes.push({ productId, type: "out", qty: Number(v.out) });
    }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.75)", zIndex: 50 }}>
      <div className="w-full max-w-lg rounded-lg p-4 max-h-[90vh] flex flex-col" style={{ background: palette.panel, border: `1px solid ${palette.borderStrong}` }}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="display-font text-sm font-semibold flex items-center gap-1"><Layers size={14} color={palette.accent} /> Carga en lote</h3>
          <button onClick={onCancel}><X size={16} color={palette.textMuted} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 mb-3">
          {filtered.map((p) => (
            <div key={p.id} className="rounded-lg p-2" style={{ background: palette.panelAlt, border: `1px solid ${palette.border}` }}>
              <p className="text-sm mb-1 truncate">{p.name} <span style={{ color: palette.textMuted }}>· stock {p.qty}</span></p>
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="text-xs" style={{ color: palette.textMuted }}>Entrada</label>
                  <input
                    type="number"
                    value={rows[p.id]?.in || ""}
                    onChange={(e) => setRow(p.id, "in", e.target.value)}
                    className="w-full text-sm px-2 py-1 rounded mt-0.5"
                    style={{ background: palette.panel, border: `1px solid ${palette.border}`, color: palette.success }}
                  />
                </div>
                <div className="flex-1">
                  <label className="text-xs" style={{ color: palette.textMuted }}>Salida</label>
                  <input
                    type="number"
                    value={rows[p.id]?.out || ""}
                    onChange={(e) => setRow(p.id, "out", e.target.value)}
                    className="w-full text-sm px-2 py-1 rounded mt-0.5"
                    style={{ background: palette.panel, border: `1px solid ${palette.border}`, color: palette.danger }}
                  />
                </div>
              </div>
            </div>
          ...
          {filtered.length === 0 && (
            <p className="text-sm py-6 text-center" style={{ color: palette.textMuted }}>No hay productos que coincidan.</p>
          )}
        </div>
        ...
        <button
          ...
          className="w-full py-2 rounded text-sm font-medium"
          style={{ background: palette.accent, color: palette.accentText, opacity: changeCount === 0 || applying ? 0.6 : 1 }}
        >
          {applying ? "Aplicando..." : `Aplicar ${changeCount} cambio(s)`}
        </button>
      </div>
    </div>
  );
