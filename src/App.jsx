...
export default function App() {
  ...
  async function fetchAll() {
    ...
    setLoading(true);
    try {
      const [catsRes, prodsRes, movsRes] = await Promise.all([
        supabase.from("categories").select("*").order("name"),
        supabase.from("products").select("*").order("name"),
        supabase.from("movements").select("*").order("created_at", { ascending: false }).limit(200),
      ]);
      if (catsRes.error || prodsRes.error || movsRes.error) {
        setErrorMsg(
          (catsRes.error || prodsRes.error || movsRes.error).message ||
            "No se pudo conectar con la base de datos. Revisa tus variables de entorno."
        );
      } else {
        setErrorMsg("");
  ...
  function handleScanResult(code) {
    ...
    if (found) {
      ...
      setEditingProduct({ sku: "", name: "", category_id: categories[0]?.id || "", qty: 0, min: 0, price: 0, supplier: "", barcode: code });
  ...
  return (
    <div style={{ background: PALETTE.bg, color: PALETTE.text, minHeight: "100vh", fontFamily: "Inter, system-ui, sans-serif" }} className="w-full p-4">
      ...
      <div className="flex items-center justify-between mb-4 pb-4" style={{ borderBottom: `1px solid ${PALETTE.border}` }}>
        ...
        <div className="flex gap-2">
          <button
            ...
            style={{
              background: tab === "inventario" ? PALETTE.accent : "transparent",
              color: tab === "inventario" ? PALETTE.accentText : PALETTE.textMuted,
              border: `1px solid ${tab === "inventario" ? PALETTE.accent : PALETTE.border}`,
            }}
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded text-sm font-medium"
            style={{
              background: tab === "movimientos" ? PALETTE.accent : "transparent",
              color: tab === "movimientos" ? PALETTE.accentText : PALETTE.textMuted,
              border: `1px solid ${tab === "movimientos" ? PALETTE.accent : PALETTE.border}`,
            }}
          >
...
function ProductModal({ product, categories, palette, onCancel, onSave }) {
  const [form, setForm] = useState(
    product || { sku: "", name: "", category_id: categories[0]?.id || "", qty: 0, min: 0, price: 0, supplier: "", barcode: "" }
  );

  function handleSave() {
    if (!form.name.trim() || !form.sku.trim()) return;
    onSave({ ...form, qty: Number(form.qty), min: Number(form.min), price: Number(form.price) });
  }
      ...
      <div className="w-full max-w-sm rounded-lg p-4 max-h-[90vh] overflow-y-auto" style={{ background: palette.panel, border: `1px solid ${palette.borderStrong}` }}>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <Field label="Precio" type="number" value={form.price} onChange={(v) => setForm({ ...form, price: v })} palette={palette} />
          </div>
